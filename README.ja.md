...
- **デプロイ設定リファレンスの拡充**：`.env.example` に CORS 許可リスト（`CORS_ALLOW_ORIGINS`）、チャンネル複数キーの暗号化マスターキー（`CHANNEL_KEY_ENCRYPTION_KEY`）、サーキットブレーカー設定、Prometheus メトリクススイッチ（`METRICS_ENABLED`）、上流リクエストのタイムアウト / プロキシ、全体 API・Web レート制限などの項目を追加し、意味と既定値を明記。
- **多言語文言の修正**：英語 UI で「設定 → コンテンツ設定 → 概要」の入力欄プレースホルダーが内部キー名のまま表示される問題を修正し、英語ロケールから冗長な 5 件のキーを削除。
- **ドキュメント更新**：英語 / 日本語 README に v1.1.1・v1.1.2 の更新履歴を追加。Wiki の「モデル倍率」に推論倍率の節を追加し、「Docker デプロイガイド」に新しい環境変数を追記。

**エンジニアリング**

- **CI にフロントエンドビルド検証を追加**：管理画面 3 テーマのビルドタスクを追加し、フロントエンドの依存解決異常をコミット / プルリクエスト段階で検出（リリース時の障害を防止）。
- **ビルド環境の統一**：リリースワークフローの Node を 16 から 20 に更新（コンテナイメージと一致）、Go のバージョン制約を 1.22 に引き締め、CI がプルリクエストでも実行されるように。
- **コード整理**：管理画面に残っていたデバッグ出力を削除。

### v1.1.1（2026-09-10）

**セキュリティ修正**

- **フロントエンド XSS 対策**：管理画面のすべてのリッチテキスト（ホーム / 概要 / お知らせ / フッター）と Markdown レンダリングを HTML サニタイズし、スクリプト注入や悪意あるリンクプロトコルを遮断。
- **CORS の強化**：「任意のオリジン＋認証情報」を許容していた設定不備を修正。CORS 許可リストを追加し、設定時は許可リスト内のオリジンのみ認証情報を送信可能、未設定時は匿名のクロスオリジンのみ許可。

**改善**

- **モバイル対応**：管理画面が小さい画面に最適化（テーブルはカード化、モーダルは全幅、フォームは 1 列、入力時のフォーカスズーム防止、横スクロールなし）。デスクトップのレイアウトと操作は従来どおり。
- **イメージビルドの安定化**：フロントエンドの依存バージョンを固定し、ビルド環境ごとの依存解決の差異によるリリースイメージのビルド失敗を修正。

### v1.1.0（2026-08-28）

**機能強化**

- **スマートルーティング＆サーキットブレーカー**：「グループ＋モデル」単位でチャンネル選択戦略（優先度 / 重み / 低遅延優先 / ランダム）を設定可能、グループ・モデルとも `*` ワイルドカードに対応。連続失敗したチャンネルを一時的にスキップし、クールダウン後に自動復帰するサーキットブレーカーを追加。
- **仮想モデル**：複数の実モデルを 1 つの入口に集約し、リクエスト時に重みに応じて実モデルを自動選択。モデルバージョンの段階的リリースやフォールバックに便利。
- **チャンネル複数キー負荷分散**：1 つのチャンネルに複数の API キーを設定し、重み付きランダムで配分。認証失敗時に該当キーを自動隔離、チャンネルテスト成功後に自動復帰。キーは暗号化保存され、UI にはマスク済みサフィックスのみ表示。
- **Prometheus メトリクス**：標準 Prometheus 形式のメトリクス（リクエスト数 / レイテンシ / サーキット状態など）を出力する `/metrics` エンドポイントを追加。システム設定で ON/OFF 可能。
- **推論トークンの独立課金**：モデル単位で推論（reasoning）トークンの倍率を設定できる「推論倍率」を追加。未設定モデルは出力倍率に自動フォールバック。
- **チャンネル健全性診断**：0-100 の健全性スコア、サーキット状態、最近のプローブ記録をチャンネル編集ページに表示。
- **管理 UI 拡張（3 テーマ）**：システム設定に「スマートルーティング」タブ（ルーティングポリシー＋仮想モデル）、運用設定に「推論倍率」、チャンネル編集ページに「キー管理」と健全性ステータスを追加。default / air / berry の 3 テーマで同期。

### v1.0.8（2026-08-21）

**バグ修正**

- **グローバル倍率 map の並行読み書き panic を修正**：管理者がモデル/キャッシュ倍率（`CompletionRatio`/`CacheHitRatio`/`CacheWriteRatio`）を実行時更新すると課金パスと並行してグローバル map を変更し、Go map の並行読み書きで panic していました。全 Get/Update/JSON シリアライズ経路に読み書きロックを追加しました。
- **PostgreSQL の初回応答遅延統計の型変換失敗を修正**：`AvgFirstTokenTime` の `AVG` は PostgreSQL では numeric を返し、int64 への Scan が失敗していました。DB 互換の AVG 式（PG `::bigint`、MySQL/SQLite は `ROUND` 後 CAST）に変更しました。
- **フロントエンドのバージョン欠落を修正**：VERSION ファイルを Docker マルチステージビルドで引き継ぎ、フロントエンドの `REACT_APP_VERSION` が正しいバージョンを表示するようになりました。
- **ストリーミング非同期課金がリクエスト取消で失敗する問題を修正**：ストリーミング課金は goroutine 内でリクエスト ctx を使用しており、リクエスト終了後に ctx がキャンセルされて課金が失われていました。`context.WithoutCancel` に切り替えました。
- **SQLite/MySQL の平均値切り捨ての不整合を修正**：`ROUND` による丸め処理を統一し、SQLite の浮動小数点直接切り捨てと MySQL の差を解消しました。
- **deepl アダプタのレスポンスボディクローズを修正**：`resp.Body.Close()` の呼び出し位置を修正し、ボディが正しく閉じられるようにしました。
- **画像生成ログの請求明細欠落を修正**：画像生成ログの `BillingDetail` に `Model` フィールドと倍率/クォータ内訳を追加し、逆算検証が可能になりました。

**機能改善**

- **統計インターフェースがチャンネル別フィルタに対応**：`SumUsedToken`/`SumUsedQuota`/`CountConsumeLogs`/`SearchLogsByModel`/`AvgFirstTokenTime` が `channel` パラメータを受け取り、チャンネル別の正確な集計が可能になりました。
- **Air テーマの「データダッシュボード」メニューを常時表示**：「データエクスポート」スイッチから分離し、モデル分析の入口が隠れなくなりました。
- **初回ログイン時の強制パスワード変更**：管理者がパスワードを作成/リセットした後、ユーザーは初回ログイン時に変更が必須になります（3 テーマとも強制変更モーダル）。アカウントセキュリティを向上させました。
- **ログテーブルの横配置と横スクロール（UI）**：ログテーブルが横スクロールと最小列幅に対応。行展開の詳細は横配置になり、長いフィールドがレイアウトを圧迫しなくなりました。
- **Air テーマの UI 統一**：14 ファイルを semantic-ui-react から Semi Design に移行し、ライブラリ混在を解消（管理者設定ページは Semantic UI Form コントロールを維持、外枠レイアウトのみ統一）。

### v1.0.7（2026-08-18）

**機能改善**

- **ログモジュールの全面強化**：3 テーマ（default/air/berry）のログページにサマリーカード（総消費 / 総トークン / リクエスト数 / 平均初回応答遅延）と「所要時間」「初回応答」「グループ」「IP」列、行展開による詳細表示（リクエスト ID、キャッシュヒット/書き込み、初回応答遅延、総所要時間、構造化された請求明細）を追加。
- **ホームダッシュボードの強化**：アカウント / 利用 / リソース / パフォーマンス の 4 グループのサマリーカードを追加。
- **初回応答遅延（TTFT）の計測**：ストリーミングリクエストで最初の非空コンテンツトークン到着時刻（ms）を記録。ログの「初回応答」列とダッシュボードの平均値で表示。非ストリーミングとツール呼び出しのみのリクエストは対象外。
- **構造化された請求明細と逆算検証**：各消費ログに構造化された請求内訳（モデル倍率 / グループ倍率 / 出力倍率 / キャッシュヒット割引 / キャッシュ書き込み加算 / チャンネル種別 / 各分量）を保存し、行展開で完全な請求式を表示。表示上のキャッシュヒット数と実請求の不一致も修正。
- **モデル分析ダッシュボード**：モデル別にリクエスト数、消費、トークン、平均初回応答遅延、平均所要時間を集計。管理者は全ユーザー、一般ユーザーは自分のみ表示（`/api/log/model-analysis` と `/api/log/self/model-analysis`）。
...
28. 🆕 **スマートルーティング＆サーキットブレーカー** — 「グループ＋モデル」単位で選択戦略（優先度/重み/低遅延/ランダム）、`*` ワイルドカード対応。連続失敗チャンネルを一時スキップし自動復帰。
29. 🆕 **仮想モデル** — 複数の実モデルを 1 つの入口に集約、重みに応じて実モデルを自動選択。
30. 🆕 **チャンネル複数キー負荷分散** — 1 チャンネルに複数キーを重み付き配分、認証失敗で隔離・プローブ成功で自動復帰。キーは暗号化保存。
31. 🆕 **Prometheus メトリクス** — 標準 Prometheus 形式の指標を出力する `/metrics` エンドポイント（システム設定で ON/OFF 可能）。
32. 🆕 **推論トークン独立課金** — モデル単位で推論倍率を設定、未設定は出力倍率にフォールバック。
33. 🆕 **チャンネル健全性診断** — 0-100 健全性スコア、サーキット状態、最近のプローブ記録をチャンネル編集ページに表示。
...
上のイメージをプルできない場合は、Docker Compose デプロイ（下記）をお試しください。
...
本イメージの v1.0.0 は songquanpeng/one-api の v0.6.7 をベースに開発され、最適化と修正が加えられています。以降のバージョンは独立して更新されています。
