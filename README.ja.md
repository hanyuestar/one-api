...
- **グローバル倍率 map の並行読み書き panic を修正（H1）**：管理者がモデル/キャッシュ倍率（`CompletionRatio`/`CacheHitRatio`/`CacheWriteRatio`）を実行時更新すると課金パスと並行してグローバル map を変更し、Go map の並行読み書きで panic していました。全 Get/Update/JSON シリアライズ経路に読み書きロックを追加しました。
- **PostgreSQL の初回応答遅延統計の型変換失敗を修正（M1）**：`AvgFirstTokenTime` の `AVG` は PostgreSQL では numeric を返し、int64 への Scan が失敗していました。DB 互換の AVG 式（PG `::bigint`、MySQL/SQLite は `ROUND` 後 CAST）に変更しました。
- **フロントエンドのバージョン欠落を修正（M3）**：VERSION ファイルを Docker マルチステージビルドで引き継ぎ、フロントエンドの `REACT_APP_VERSION` が正しいバージョンを表示するようになりました。
- **ストリーミング非同期課金がリクエスト取消で失敗する問題を修正（L6）**：ストリーミング課金は goroutine 内でリクエスト ctx を使用しており、リクエスト終了後に ctx がキャンセルされて課金が失われていました。`context.WithoutCancel` に切り替えました。
- **SQLite/MySQL の平均値切り捨ての不整合を修正（L3）**：`ROUND` による丸め処理を統一し、SQLite の浮動小数点直接切り捨てと MySQL の差を解消しました。
- **deepl アダプタのレスポンスボディクローズを修正（L4）**：`resp.Body.Close()` の呼び出し位置を修正し、ボディが正しく閉じられるようにしました。
- **画像生成ログの請求明細欠落を修正（L7）**：画像生成ログの `BillingDetail` に `Model` フィールドと倍率/クォータ内訳を追加し、逆算検証が可能になりました。

**機能改善**

- **統計インターフェースがチャンネル別フィルタに対応（M2）**：`SumUsedToken`/`SumUsedQuota`/`CountConsumeLogs`/`SearchLogsByModel`/`AvgFirstTokenTime` が `channel` パラメータを受け取り、チャンネル別の正確な集計が可能になりました。
- **Air テーマの「データダッシュボード」メニューを常時表示（M4）**：「データエクスポート」スイッチから分離し、モデル分析の入口が隠れなくなりました。
- **初回ログイン時の強制パスワード変更（L1）**：管理者がパスワードを作成/リセットした後、ユーザーは初回ログイン時に変更が必須になります（3 テーマとも強制変更モーダル）。アカウントセキュリティを向上させました。
