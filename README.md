...
- **部署配置说明补齐**：`.env.example` 新增跨域来源白名单（`CORS_ALLOW_ORIGINS`）、渠道多 Key 加密主密钥（`CHANNEL_KEY_ENCRYPTION_KEY`）、渠道熔断参数、Prometheus 指标开关（`METRICS_ENABLED`），以及上游请求超时 / 代理、全局 API 与网页限流等常用配置项，均标注含义与默认值。
- **多语言文案修正**：修复英文界面下「设置 → 内容设置 → 关于」输入框提示未翻译（显示为内部键名）的问题，并清理英文语言包中 5 处冗余文案键。
- **文档更新**：英文 / 日文 README 补齐 v1.1.1 与 v1.1.2 更新日志；Wiki「模型倍率说明」新增推理倍率章节，「Docker 部署指南」补充新增环境变量。

**工程质量**

- **CI 增加前端构建校验**：新增管理界面全部主题的前端构建任务，前端依赖解析异常在提交 / 合并请求阶段即可发现，不再拖到正式发布才暴露。
- **构建环境统一**：发布流程的 Node 版本由 16 升级至 20（与容器镜像保持一致），Go 版本约束收紧至 1.22；CI 支持合并请求（Pull Request）触发。
- **代码整洁**：移除管理界面遗留的调试输出。
...
- **前端 XSS 防护**：管理界面所有富文本（首页内容 / 关于 / 公告 / 页脚）与 Markdown 渲染均经过 HTML 净化，拦截脚本注入与恶意链接协议，防止跨站脚本攻击。
...
- **管理界面增强**：系统设置新增「智能路由」页（路由策略 + 虚拟模型管理），运营设置新增「推理倍率」，渠道编辑页新增「Key 管理」与健康状态；default / air / berry 各主题同步。

### v1.0.8（2026-08-21）

**Bug 修复**

- **修复全局倍率 map 并发读写 panic**：运行时管理员更新模型/缓存倍率（`CompletionRatio`/`CacheHitRatio`/`CacheWriteRatio`）会并发修改全局 map，而计费路径并发读取，Go map 并发读写直接 panic。新增读写锁保护所有 Get/Update/JSON 序列化路径。
- **修复 PostgreSQL 首字延迟统计类型转换失败**：`AvgFirstTokenTime` 的 `AVG` 在 PostgreSQL 返回 numeric，Scan 到 int64 失败。改为跨库兼容的 AVG 表达式（PG `::bigint`、MySQL/SQLite `ROUND` 后 CAST）。
- **修复前端版本号缺失**：VERSION 文件纳入 Docker 多阶段构建传递，前端 `REACT_APP_VERSION` 正确显示当前版本。
- **修复流式异步计费因请求取消而失败**：流式计费在 goroutine 中使用请求 ctx，请求结束后 ctx 被取消导致计费丢失。改用 `context.WithoutCancel` 脱离取消链。
- **修复 SQLite/MySQL 统计均值截断不一致**：统一使用 `ROUND` 四舍五入，消除 SQLite 浮点直接截断与 MySQL 的差异。
- **修复 deepl 适配器响应体关闭逻辑**：修正 `resp.Body.Close()` 的调用，确保响应体正确关闭。
- **修复图片生成日志缺计费明细**：图片生成日志 `BillingDetail` 补全 `Model` 字段及倍率/额度明细，计费反查可读。

**功能优化**

- **统计接口支持按渠道筛选**：`SumUsedToken`/`SumUsedQuota`/`CountConsumeLogs`/`SearchLogsByModel`/`AvgFirstTokenTime` 统一增加 `channel` 参数，按渠道统计更精准。
- **air 主题「数据看板」菜单对所有用户常驻**：解耦「数据导出」开关，模型分析入口不再被隐藏。
- **新增强制改密**：管理员创建或重置密码后，用户首次登录必须修改密码（前端强制改密弹窗），提升账号安全。
- **日志列表横排与横向滚动（UI）**：日志表格支持横向滚动与最小列宽，行展开详情改为横排，长字段不再挤压布局。
...
- **日志模块全面增强（对齐专业 API 分发平台）**：全部主题（default/air/berry）日志页新增顶部摘要卡（总消耗额度 / 总 Token / 请求数 / 平均首字延迟），新增「用时」「首字」「分组」「IP」列，并支持行展开查看请求详情（Request ID、缓存命中/写入、首字延迟、总耗时、结构化计费明细）。
- **首页数据看板增强**：首页新增「账户数据 / 使用统计 / 资源消耗 / 性能指标」四组摘要卡，快速掌握账户与资源概况。
- **新增首字延迟（TTFT）统计**：流式请求记录首个非空内容 token 的到达时刻（毫秒），日志页「首字」列直观展示、看板展示平均首字延迟；非流式与纯工具调用请求不记录，避免语义混淆。
- **计费明细结构化与公式反查**：每条消费日志新增结构化计费明细（模型倍率 / 分组倍率 / 输出倍率 / 缓存命中折扣 / 缓存写入加价 / 渠道类型 / 各计费分量），日志展开可查看完整计费公式，支持对账与一致性校验；同步修正缓存命中展示与真实计费口径不一致的问题。
- **新增模型分析看板**：按模型聚合请求数、消耗额度、Token、平均首字延迟、平均耗时，管理员查看全域、普通用户查看自身（`/api/log/model-analysis` 与 `/api/log/self/model-analysis`）。

### v1.0.6（2026-08-17）

**Bug 修复**

- **修复 Anthropic / AWS Bedrock Claude 缓存计费严重少计**：Anthropic 的 `input_tokens` 不包含 `cache_read`/`cache_creation`（二者独立统计），此前计费公式按 OpenAI 口径（`prompt_tokens` 已含缓存 token）对缓存 token 重复扣减，导致所有走缓存的 Claude 请求少计约 60%。现已将缓存读取/写入 token 并入 `PromptTokens`，与 OpenAI 口径统一（流式与非流式均已修复）。
- **修复普通用户越权指定渠道（安全漏洞）**：`/v1/oneapi/proxy/:channelid/*` 路由此前任何登录令牌都可强制指定任意渠道，存在绕过分组/模型分配、占用管理员渠道额度或调用无权使用模型的风险。现已与令牌 key 后缀规则对齐：仅管理员可指定渠道。
- **修复缓存计费防御性修正篡改日志数据**：当缓存 token 数超过输入 token 总数时，防御性回退仅作用于计费计算，不再改写落库的缓存命中/写入统计。
- **修复配额扣减并发竞态（TOCTOU）**：令牌/用户额度预扣改为主库原子条件更新（`WHERE remain_quota >= ?` + 影响行数校验），高并发下不再出现超额扣减为负数的问题。
- **修复 API 令牌 Key 弱随机**：令牌 Key 由 `math/rand`（时间种子）改为 `crypto/rand` 密码学安全随机生成，消除前 16 字符可预测风险。
- **修复会话类型断言 panic**：鉴权中间件对 session 字段增加类型断言 `ok` 校验，畸形会话不再触发 panic。
- **修复工具调用参数解析**：Anthropic 渠道工具调用 `Arguments` 类型断言增加保护，JSON 解析失败记录日志而非静默忽略。

**功能优化**

- **Air 主题渠道表恢复「分组」列**：与 default/berry 主题对齐，渠道列表可查看所属分组。
- **清理 `/chat` 死路由**：删除 default/air 主题残留的 `/chat` 路由与 air 侧边栏孤儿映射，与 berry 主题保持一致。
- **缓存字段映射收敛**：OpenAI 兼容渠道的缓存命中字段统一由计费兜底逻辑处理，消除重复映射逻辑。
