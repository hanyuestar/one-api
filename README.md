...
- **新增强制改密**：管理员创建或重置密码后，用户首次登录必须修改密码（三主题强制改密弹窗），提升账号安全。
...
- **日志模块全面增强（对齐专业 API 分发平台）**：三套主题（default/air/berry）日志页新增顶部摘要卡（总消耗额度 / 总 Token / 请求数 / 平均首字延迟），新增「用时」「首字」「分组」「IP」列，并支持行展开查看请求详情（Request ID、缓存命中/写入、首字延迟、总耗时、结构化计费明细）。
